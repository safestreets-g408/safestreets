#!/usr/bin/env python3
"""
Test script for Simple CNN road classifier
"""
import os
import sys
import time
...
# Import the Simple CNN road classifier
try:
    from src.utils.simple_road_classifier import validate_road_image, load_road_classifier
    print("Testing Simple CNN road classifier...")
except ImportError:
    print("Simple CNN road classifier not found, trying original...")
    from src.utils.road_classifier import validate_road_image, load_road_classifier
    print("Testing original road classifier...")
...
def test_road_classifier():
    ...
    test_images = [
        ...
        {'path': 'static/uploads/road_classification_1751739870.jpg', 'expected': True, 'name': 'Road image 2'},
        {'path': 'static/uploads/non_road1.jpg', 'expected': False, 'name': 'Non-road image 1'}
    ]
    ...
    if not os.path.exists(test_images[0]['path']):
        ...
        # Create another sample road image with asphalt texture
        road_img2 = Image.new('RGB', (640, 480), (120, 120, 120))
        road_img2.save(test_images[1]['path'])
        
        # Create a non-road image (bright colors)
        non_road_img = Image.new('RGB', (640, 480), (0, 200, 50))
        non_road_img.save(test_images[2]['path'])
    ...
    for test_img in test_images:
        try:
            ...
            if 'heuristic_features' in result:
